...
/// A small, dependency-free SHA-256 used to hash file contents for the conflict
/// detector. We avoid `CryptoKit` so the core remains importable without Foundation
/// extensions that vary across platforms.
public enum SHA256Hash {

    public static func hex(of string: String) -> String {
        hex(of: Array(string.utf8))
    }
    ...
    public static func compute(_ message: [UInt8]) -> [UInt8] {
        ...
        for i in (0..<8).reversed() {
            bytes.append(UInt8((bitLength >> (UInt64(i) * 8)) & 0xff))
        }
